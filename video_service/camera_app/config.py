...
# MediaMTX 是系统依赖；优先使用 PATH，标准手工部署位置为 /usr/local/bin。
DEFAULT_MEDIAMTX_BINARY = Path(
    shutil.which("mediamtx") or "/usr/local/bin/mediamtx"
)
...
class CameraConfig:
    ...
    def validate(self) -> None:
        ...
        try:
            ...
            raise ValueError("推流展示IP必须是有效的 IPv4 地址") from exc
        if address.version != 4 or address.is_unspecified:
            raise ValueError("推流展示IP必须是具体的 IPv4 地址，不能使用 0.0.0.0")
