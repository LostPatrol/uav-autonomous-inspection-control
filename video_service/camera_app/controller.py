...
from .onboard_config import DEFAULT_LENS_CONFIG, load_lens_controls
...
class CameraController:
    ...
    def __init__(
        ...
        *,
        ...
        mediamtx_binary: Path = DEFAULT_MEDIAMTX_BINARY,
        ffmpeg_binary: str | None = None,
        ffprobe_binary: str | None = None,
        v4l2_ctl_binary: str | None = None,
        lens_config_path: Path | None = DEFAULT_LENS_CONFIG,
        initial_config: CameraConfig | None = None,
        persist_config: bool = True,
        strict_mode_probe: bool = True,
    ) -> None:
        ...
        self.v4l2_ctl_binary = (
            v4l2_ctl_binary or shutil.which("v4l2-ctl") or "v4l2-ctl"
        )
        self.lens_config_path = Path(lens_config_path) if lens_config_path else None
        self._persist_config = bool(persist_config)
        self._strict_mode_probe = bool(strict_mode_probe)
        ...
        self._snapshot_counters = {"manual": 0, "gcs": 0}
    ...
    def request_snapshot(
        self, *, kind: str = "manual", photo_no: str = ""
    ) -> dict[str, Any]:
        """逐条从本机 RTSP 保存带时间、来源及可选 photoNo 的 JPG。"""
        with self._snapshot_lock:
            ...
            filename = self._snapshot_filename(kind=kind, photo_no=photo_no)
    ...
    def build_ffmpeg_command(
        ...
        *,
        ...
        # FFmpeg 4.4 只有 setts 的 pts/dts 选项；用输入 time-base 表达式重建 CFR，
        # 同一命令可在飞机 Ubuntu 22.04 和开发机 Ubuntu 24.04 上运行。
        timestamp_filter = (
            f"setts=pts=N/({frame_rate}*TB):dts=N/({frame_rate}*TB)"
        )
        rtsp_output = (
    ...
    def _apply_lens_controls(self, config: CameraConfig) -> None:
        ...
        if self.lens_config_path is None:
            return
        try:
            controls = load_lens_controls(self.lens_config_path)
        except ValueError as exc:
    ...
    def _snapshot_filename(self, *, kind: str, photo_no: str) -> str:
        """生成可追溯且不覆盖旧文件的三类协议文件名。"""
        stamp = datetime.now().strftime("%Y%m%d-%H%M%S-%f")
        if kind == "upstream":
            # percent 编码对路径分隔符等字符可逆，协议原值仍保留在 ROS 结果字段中。
            encoded = quote(str(photo_no), safe="-_.~") or "%00"
            suffix = f"photoNo-{encoded}"
        elif kind in {"gcs", "manual"}:
            self._snapshot_counters[kind] += 1
            suffix = f"{kind}-{self._snapshot_counters[kind]}"
        else:
            raise CameraServiceError(f"未知抓拍来源：{kind}")
        return f"snapshot-{stamp}-{suffix}.jpg"
    ...
    def _monitor_ffmpeg(self, process: subprocess.Popen[str]) -> None:
        ...
        with self._state_lock:
            ...
            if self._state in {"starting", "running"}:
                ...
                threading.Thread(
                    target=self._cleanup_failed_pipeline,
                    args=(process,),
                    name="camera-service-failure-cleanup",
                    daemon=True,
                ).start()

    def _cleanup_failed_pipeline(self, failed_process: subprocess.Popen[str]) -> None:
        """FFmpeg 意外退出后释放本服务的 MediaMTX/端口并保留 error 诊断。"""
        with self._operation_lock:
            with self._state_lock:
                if self._state != "error" or failed_process is not self._ffmpeg:
                    return
            self._stop_processes(finalize_recording=True)
